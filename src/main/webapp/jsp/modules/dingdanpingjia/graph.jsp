...
<body>
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow min-h200">
							...
							<!--<div id="main" style="width: 900px;height:600px;"></div>-->
																																																	<div id="yonghupingfenMain" class="graph" style="width: 900px;height:600px;"></div>
																																																																																																																						</div>
					</div>
				</div>
			</div>
	...
	var pageType = "graph";
							...
							var yonghupingfenValArr = [];
	var yonghupingfenNameArr = [];
	var yonghupingfenMapList = [];
																	var valueArr = []
			var graphType = "pie";
	
							var yonghupingfenVal = '';
			...
			series : [
				{
					...
					data:[          // 数据数组，name 为数据项名称，value 为数据项值
																																										{value:yonghupingfenVal, name:'用户评分'},
																																																																																																					]
				}
			]
		})
	...
	// 需要调group的字段，一个字段一个统计图
							function yonghupingfenDraw(){
		// 基于准备好的dom，初始化echarts实例
		var myChart = echarts.init(document.getElementById('yonghupingfenMain'),'macarons');
			...
			series : [
				{
					...
					type: 'pie',    // 设置图表类型为饼图
					radius: '55%',  // 饼图的半径，外半径为可视区尺寸（容器高宽中较小一项）的 55% 长度。
					data: yonghupingfenMapList   // 数据数组，name 为数据项名称，value 为数据项值
				}
			]
		})
			}
	...
	function groupDataBind(list,colName){
																			if(colName == "yonghupingfen"){ 
											for(var i=0;i<list.length;i++){
							var map = {value: list[i].total,name: list[i].yonghupingfen}
							yonghupingfenMapList.push(map)
						}
										yonghupingfenDraw()
				}
